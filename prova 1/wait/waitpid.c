...
int main(void){
    int orig_pid;
    pid_t clone_pid[2];
    int valor_retorno, exe_status;
    time_t t;

    orig_pid = getpid();

    printf("Em execução do processo original...\n");
    ...
    if(getpid() == orig_pid){
        ...
        if(getpid() == orig_pid){
            ...
            printf("REcebi de %d o valor de %d\n", clone_pid[0], WEXITSTATUS(temp1));
            waitpid(clone_pid[1], &temp2,0);
            printf("REcebi de %d o valor de %d\n", clone_pid[1],WEXITSTATUS(temp2));
        }
    }if(getpid() != orig_pid){
        srand(getpid());
        sleep(3);
